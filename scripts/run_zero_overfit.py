...
from neuroxo.algorithms.zero import run_overfit
...
def main():
    ...
    parser.add_argument('--deterministic_by_policy', required=False, action='store_true', default=True)
    parser.add_argument('--reduced_search', required=False, action='store_true', default=False)

    parser.add_argument('--lr_init', required=False, type=float, default=4e-3)
    parser.add_argument('--n_dec_steps', required=False, type=int, default=2)

    parser.add_argument('--n_epochs', required=False, type=int, default=20)
    parser.add_argument('--batch_size', required=False, type=int, default=128)
    parser.add_argument('--n_data_epochs_train', required=False, type=int, default=8)

    parser.add_argument('--n_val_games', required=False, type=int, default=100)

    parser.add_argument('--moving', required=False, action='store_true', default=False)
    ...
    log_dir = logs_path / f'logs_overfit_{n_agents}'
    log_dir.mkdir(exist_ok=True)
    logger = SummaryWriter(log_dir=log_dir)

    configs_path = exp_path / 'configs'
    configs_path.mkdir(exist_ok=True)
    save_json(config, configs_path / f'config_overfit_{n_agents}.json')
    ...
    run_overfit(player, player_best, logger, exp_path, n_data_epochs_train=args.n_data_epochs_train,
                n_epochs=args.n_epochs, n_val_games=args.n_val_games, batch_size=args.batch_size,
                lr_init=args.lr_init, epoch2lr=epoch2lr, moving=args.moving)
